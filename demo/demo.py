...
class VisualizationDemo(object):
    def __init__(self, cfg, vis_highest_scoring=False, output_dir="./vis"):
        """
        Args:
            cfg (CfgNode):
    ...
    def run_on_image(self, image, focal_length=10.0):
        ...
        image = image[:, :, ::-1] # take every thing backwards BGR -> RGB
        ...
        if "instances" in predictions:
            ...
            if self.vis_highest_scoring:
                ...
                # filter prob < 0.8 scores
                det_ids_filter = []
                
                det_ids = range(len(scores))
                for idx in range(len(scores)):
                    if scores[idx] > 0.8:
                        det_ids_filter.append(idx)
                        

            # for det_id in det_ids:
            for det_id in det_ids_filter:
